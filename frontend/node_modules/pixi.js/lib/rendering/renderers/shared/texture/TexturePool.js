...
var ScreenSizeRegistry = require('./utils/ScreenSizeRegistry.js');

"use strict";
let count = 0;
const maxKeyDimension = 32767;
const scaleBit = 4294967296;
const formatBit = 8589934592;
const formatIds = /* @__PURE__ */ Object.create(null);
let nextFormatId = 0;
...
class TexturePoolClass {
  ...
  constructor(textureOptions) {
    ...
    this._screens = new ScreenSizeRegistry.ScreenSizeRegistry();
   ...
   */
  get textureStyle() {
    deprecation.deprecation(deprecation.v8_21_0, "TexturePool.textureStyle is no longer used, pooled textures carry their own style. Pass scaleMode in the request instead.");
    if (!this._textureStyle) this._textureStyle = new TextureStyle.TextureStyle(this.textureOptions);
  ...
  get enableFullScreen() {
    deprecation.deprecation(deprecation.v8_21_0, "TexturePool.enableFullScreen is no longer used, the pool sizes textures to the screens registered with setScreenSize.");
  ...
  createTexture(...args) {
    ...
    if (typeof options === "number") {
      deprecation.deprecation(deprecation.v8_21_0, "TexturePool.createTexture params are now an options object. See params: { width, height, antialias, autoGenerateMipmaps }");
      options = {
        width: options,
        height: args[1],
    ...
    const format = options.format ?? this.textureOptions.format ?? TextureSource.TextureSource.defaultOptions.format;
    const scaleMode = options.scaleMode ?? this.textureOptions.scaleMode ?? TextureStyle.TextureStyle.defaultOptions.scaleMode;
  ...
  getOptimalTexture(...args) {
    let options = args[0];
    if (typeof options === "number") {
      deprecation.deprecation(deprecation.v8_21_0, "TexturePool.getOptimalTexture params are now an options object. See params: { width, height, resolution, antialias, autoGenerateMipmaps }");
      options = {
        width: options,
        height: args[1],
        resolution: args[2] ?? 1,
    ...
    if (textureWidth > maxKeyDimension || textureHeight > maxKeyDimension) {
      warn.warn(`TexturePool: ${textureWidth}x${textureHeight} is larger than the ${maxKeyDimension}px pool key limit, textures of this size may be pooled together`);
    }
  ...
  getOptimalSize(frameWidth, frameHeight, resolution = 1) {
    ...
    const po2Width = pow2.nextPow2(pixelWidth);
    const screenWidth = this._screens.getFittingWidth(pixelWidth);
    const po2Height = pow2.nextPow2(pixelHeight);
  ...
  returnTexture(renderTexture, resetStyle = false) {
    ...
    if (key === void 0) {
      warn.warn("TexturePool: returnTexture was passed a texture that did not come from this pool, ignoring it");
      return;
    }
  ...
  _pruneScreenTextures() {
    for (const [key, textures] of this._buckets) {
      ...
      if ((pow2.isPow2(width) || this._screens.hasWidth(width)) && (pow2.isPow2(height) || this._screens.hasHeight(height))) continue;
