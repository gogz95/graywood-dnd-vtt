<!-- src/lib/components/modals/FirstRunWizardModal.svelte -->
<!-- Initial Setup Wizard: 4-step guided onboarding for DM workstation -->
...
{#if isOpen}
  <div class="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in select-none">
    <div class="w-full max-w-xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
      <!-- Header -->
      ...
      <div class="px-6 py-3 bg-slate-950 border-t border-slate-800 flex items-center justify-between">
        ...
        {#if step < 4}
          <button
            type="button"
            onclick={() => step = (step + 1) as any}
            class="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold text-xs transition-colors shadow"
          >
            Next Step
          </button>
        {:else}
          <button
            type="button"
            onclick={finishSetup}
            class="px-5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold text-xs transition-colors shadow-lg shadow-emerald-600/30"
          >
            Launch Workstation
          </button>
        {/if}
