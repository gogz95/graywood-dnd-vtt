use aleamos_desktop_lib::{
    init_database, run_server, AppState, DEFAULT_SERVER_ADDR,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("Database initialized and migrations verified at: {:?}", db_path);
    ...
    println!("Starting embedded Axum server on http://{}", DEFAULT_SERVER_ADDR);
